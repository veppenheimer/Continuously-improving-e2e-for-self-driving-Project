...
def preprocess(frame, host_mem):
    ...
    arr = np.array(img, dtype=np.float32, copy=False) / 255.0
    np.copyto(host_mem, arr.transpose(2, 0, 1)[None, ...])

def infer(ctx, stream, devptr_in, devptr_out):
    """执行 TensorRT 推理（异步 + 零拷贝）"""
...
def main():
    ...
    try:
        while not rospy.is_shutdown():
            ...
            infer(ctx, stream, devptr_in, devptr_out)
            end_infer = time.time()
