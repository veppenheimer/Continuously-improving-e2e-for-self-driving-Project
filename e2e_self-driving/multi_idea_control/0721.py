...
PERSON_ENGINE_PATH = rospy.get_param('~person_engine_path', './person_model.engine')  # 新模型路径
...
class MotionController:
    def __init__(self):
        ...
        with open(ENGINE_PATH, 'rb') as f, trt.Runtime(TRT_LOGGER) as runtime:
            self.engine_main = runtime.deserialize_cuda_engine(f.read())
        self.context_main = self.engine_main.create_execution_context()

        # 加载person模型
        rospy.loginfo("Loading person TensorRT engine from %s ...", PERSON_ENGINE_PATH)
        with open(PERSON_ENGINE_PATH, 'rb') as f, trt.Runtime(TRT_LOGGER) as runtime:
            self.engine_person = runtime.deserialize_cuda_engine(f.read())
        self.context_person = self.engine_person.create_execution_context()
        ...
        self.stop_duration_person = rospy.get_param('~stop_duration_person', 2.0)
    ...
    def infer_trt(self,person_flag):
        context = self.context_person if person_flag else self.context_main
        context.execute_async_v2([int(self.devptr_in), int(self.devptr_out)], self.stream.handle)
        ...
        while not rospy.is_shutdown():
            ...
            if self.current_action is None and self.sign_queue:
                ...
                if sign == 'red' and not self.red_processed:
                    action = 'delay_red'; duration = self.delay_before_red;self.red_processed = True
                elif sign == 'limit':
            ...
            if self.current_action:
                ...
                if self.current_action == 'person':
                    if elapsed < self.action_duration:
                        ...
                        self.current_action = None
                        self.current_sign = None
                        self.person_flag = not self.person_flag  # 取反标志位
                ...
                if self.current_action == 'delay_limit':
                    if elapsed < self.action_duration:
                        ...
                        if frame is not None:
                            ...
                            ang_z = self.infer_trt(self.person_flag)
