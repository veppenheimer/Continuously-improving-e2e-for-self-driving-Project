...
# ==== 配置部分 ====
ENGINE_PATH   = "/home/epaicar/results/ve_0524x_simplified_fp16.engine"  # TensorRT 引擎路径
CAMERA_INDEX  = 0          # 摄像头索引
LINEAR_SPEED  = 0.5        # 固定线速度 (m/s)
...
def main():
    ...
    try:
        while not rospy.is_shutdown():
            frame_start = time.time()  # 记录整帧处理起始时间
            ...
            start = time.time()  # 推理计时开始
            context.execute_async_v2(bindings=[int(d_input), int(d_output)], stream_handle=stream.handle)
            output = np.empty((1,), dtype=np.float32)
            cuda.memcpy_dtoh_async(output, d_output, stream)
            stream.synchronize()
            end = time.time()  # 推理计时结束
            infer_time = (end - start) * 1000.0  # ms

            steering = float(output[0])
            rospy.loginfo("推理结果: %.4f，推理耗时: %.2f ms", steering, infer_time)
            ...
            frame_end = time.time()
            total_frame_time = (frame_end - frame_start) * 1000.0  # ms
            rospy.loginfo("单帧处理总时间: %.2f ms", total_frame_time)
